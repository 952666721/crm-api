...
//    }
    ...
    public void approvalContract(ApprovalQuery query) {
        ...
        // 审核状态处理（0=通过，1=拒绝）
        Integer contractStatus = query.getType() == 0 ? 2 : 3; // 2=通过，3=未通过
        ...
        approval.setContractId(contract.getId());
        approval.setComment(query.getComment()); // 前端传入的审核内容（通过/不通过原因）
        approvalMapper.insert(approval);
